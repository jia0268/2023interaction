...
void draw(){
  ...
  for(int i=0;i<26;i++){
    if(ID<typing.length() && typing.charAt(ID)-'a'==i)rect(pos[i][0],pos[i][1],43,43,5);
  ...
  fill(255,0,0);//紅色
  if(ID<typing.length())text(typed+typing.charAt(ID),50,120);//現在要打的字是
  fill(0);
  text(typed,50,120); //已經打好的字
}
String typing = "printf is a fuction";
